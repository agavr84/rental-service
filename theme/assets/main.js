...
(function () {
  ...
  function applyHeader(btn) {
    if (!titleEl || !descEl || !btn) return;
      var title = btn.getAttribute('data-hub-title') || 'Каталог объектов';
      var desc = btn.getAttribute('data-hub-description') || 'Выберите категорию для фильтрации.';
  ...
  function openNav() {
    ...
    if (navOpen) {
      ...
      navOpen.setAttribute('aria-label', 'Закрыть меню');
    }
  }
  ...
  function closeNav() {
    ...
    if (navOpen) {
      ...
      navOpen.setAttribute('aria-label', 'Открыть меню');
    }
  }
